...
async function runSearch (db, query, limit = 50) {
  ...
  settled.forEach((result, i) => {
    if (result.status === 'rejected') {
      console.error(`[adapter:${adapters[i].name}]`, result.reason && result.reason.message || result.reason)
      return
    }
  ...
  const sources = settled.map((r, i) => ({
    ...
    error: r.status === 'rejected' ? String(r.reason && r.reason.message || r.reason).slice(0, 200) : null
  }))

  const responseArticles = articles.slice(0, Number(limit) || 50)
  return { results: responseArticles, total: responseArticles.length, sources }
}
...
async function searchHandler (req, res, next) {
  try {
    ...
    const out = await runSearch(req.db, query.trim(), limit)
    res.json(out)
  } catch (err) {
    next(err)
  }
...
module.exports = { runSearch, searchHandler, ADAPTER_NAMES }
